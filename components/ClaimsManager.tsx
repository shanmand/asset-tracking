...
import { AlertCircle, Clock, CheckCircle2, History, User, FileText, ChevronRight, XCircle, Search, ShieldAlert } from 'lucide-react';
...
const ClaimsManager: React.FC<ClaimsManagerProps> = ({ isManager }) => {
  ...
  const auditLogs = MOCK_CLAIM_AUDITS.filter(a => a.claim_id === selectedClaimId);
  const driver = MOCK_LOGISTICS.find(l => l.id === selectedClaim.driver_id);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-bold text-amber-600">R {formatCurrency(8450.00)}</p>
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-bold text-emerald-600">R {formatCurrency(45900.00)}</p>
